...
Develop a static class to perform simple operations on files: displaying a file, displaying it with the comments, reverse its content, concatenate two files, and finally (for more experienced programmers), determined if the content of the file is balanced (as defined below).
...
#. A `Reverse` method that takes two `string` arguments, `arg1` and `arg2`, and write at `arg2` the content of the file located at `arg1`, reversed. If `arg1` does not exists or if a file is alrealy present at `arg2`, then the method should display an error message.
...
| ⚠ Warning |
|:---------------------------|
| You are allowed to use only `StreamReader`'s `ReadLine` and constructor, `StreamWriter`'s `WriteLine` and constructor, and the usual `Console` methods, lists, arrays, conditionals, etc. In particular, *do not* use the Linq library. |
...
Executing it with a properly implemented `TextFileHelper` missing methods should give something along the lines of:
...
- Try handling gracefully exceptions if files do not exist,
